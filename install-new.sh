...
install_binary() {
    ...
    # TODO: make these optional, dont loop over them, see: https://bun.sh/install
    S_FILES=("$HOME/.bashrc" "$HOME/.zshrc")
    for rcFile in "${S_FILES[@]}"; do
      if [ -f "$rcFile" ] && ! grep -q "export PATH=\"${DRIA_INSTALL_DIR}:\$PATH\"" "$rcFile"; then
        echo "" >> "$rcFile" # newline
        echo '# Dria Compute Launcher' >> "$rcFile"
        echo "export PATH=\"${DRIA_INSTALL_DIR}:\$PATH\"" >> "$rcFile"
      fi
    done
